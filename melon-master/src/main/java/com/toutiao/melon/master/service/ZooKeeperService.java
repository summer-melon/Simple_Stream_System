...
public class ZooKeeperService {
    ...
    public boolean jobExists(String jobName) {
        return zkConn.exists("/master/topology/" + jobName);
    }
    ...
    public synchronized void startJob(String jobName, ComputationGraph computationGraph)
        ...
        txn.create("/master/topology/" + jobName, "run".getBytes(),
                OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
        try {
            txn.commit();
        } catch (Exception e) {
    ...
    public void stopJob(String jobName) {
        String jobPath = "/master/topology/" + jobName;
    ...
    public Map<String, String> getRunningJobs() {
        List<String> jobNames = zkConn.getChildren("/master/topology");
